...
sudo docker node ls | grep -v 'Leader' | awk 'NR > 1 {print $1}' | while read line; do sudo docker node update --label-add role=worker $line; done
# can be checked via "sudo docker node ls -f "node.label=role=worker""

# label current swarm node as swarm master
sudo docker node ls | grep 'Leader' | awk '{print $1}' | while read line; do sudo docker node update --label-add role=master $line; done
